...
meses = [col for col in pg.columns if col.startswith('mes_')]

for mes in meses:
    pg[f'{mes}_numeric'] = pd.to_numeric(pg[mes], errors='coerce')
    pg[f'default_{mes}'] = pg[f'{mes}_numeric'] >= 90

pg[[col for col in pg.columns if col.startswith('default_')]].head()


# In[16]:


colunas_default = [col for col in pg.columns if col.startswith('default_mes_')]

pg['qtd_meses_default'] = pg[colunas_default].sum(axis=1)

pg['mau_pagador'] = pg['qtd_meses_default'] >= 1

pg[['ID', 'qtd_meses_default', 'mau_pagador']].head()
...
ids_com_cartao = pg['ID'].unique()

propostas_expostas = propostas[propostas['ID'].isin(ids_com_cartao)].copy()

propostas_expostas.head()


# In[18]:


base_final = propostas.merge(pg[['ID', 'mau_pagador']], on='ID', how='left')

base_final[['ID', 'mau_pagador']].head()


# In[19]:


base_final['mau_pagador'].value_counts(dropna=False)
